...
test("related link input has its own row and prompt", () => {
  const html = fs.readFileSync(
    path.join(__dirname, "..", "src", "renderer", "index.html"),
    "utf8"
  );
  ...
  assert.match(html, /id="linkInput"/);
  assert.match(html, /placeholder="相关链接"/);
  assert.match(css, /grid-template-areas:[^;]*"link"/s);
  assert.match(css, /\.link-input\s*\{[^}]*grid-area:\s*link/s);
});
...
test("editor stretches to fill the available writing row", () => {
  ...
  assert.match(css, /grid-template-rows:\s*auto\s+auto\s+300px\s+auto\s+minmax\(0,\s*1fr\)\s+auto/s);
